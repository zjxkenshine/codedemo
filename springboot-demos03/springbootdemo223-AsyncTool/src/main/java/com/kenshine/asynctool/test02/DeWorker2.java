 ...
 * @version: $
 */
public class DeWorker2 implements IWorker<String, User>, ICallback<String, User> {

    @Override
    public User action(String object, Map<String, WorkerWrapper> allWrappers) {
        System.out.println("-----------------");
        System.out.println("获取worker1的执行结果： " + allWrappers.get("first").getWorkResult());
        System.out.println("取worker1的结果作为自己的入参，并将worker1的结果加上一些东西");
        try {
            Thread.sleep(1000);
        ...
        return new User(user0.getName() + " worker2 add");
    }
